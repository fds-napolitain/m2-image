/*
Outil de compactage/d�compactage de fichier utilisant le codage de Huffman
...
/*
Calcule la fr�quence de chaque caract�re dans un fichier et trie la liste de structures
par fr�quences croissantes
*src : pointeur sur le fichier
*nbre_octets : pointeur sur une variable recevant la taille du fichier
*nbre_ascii : pointeur sur une variable recevant le nombre de caract�res
diff�rents pr�sents dans le fichier source
Retourne l'index de la premi�re structure dans la liste tri�e
*/
short huffman_calculer_frequences(FILE* src, unsigned long* nbre_octets, unsigned short* nbre_ascii)
{
    ...
    *nbre_ascii = 0;
    ...
    while ((r = fread(buffer, 1, BUF_LEN, src)) > 0)
    {
        *nbre_octets += r;
        for (i = 0; i < r; i++)
            /* incr�mentation du compteur du caract�re correspondant */
            arbre_d[buffer[i]].frequence++;
    }

    /* Chainage des structures avec une fr�quence sup�rieure � 0 */
    for (i = 0; i < 256; i++)
    ...
    /* On retourne l'index de la premi�re structure */
    return index_frequence;
}

/*
Lit les fr�quences de chaque caract�re, inscrites soit dans le fichier
compress�, soit dans un fichier de fr�quences sp�cial
*/
...
/*
Cr�e un arbre � partir d'une liste de fr�quences
index_fr�quence : idnex de la premi�re structure dans la liste *arbre_d
...
short huffman_creer_arbre(short index_frequence)
{
    ...
    /* Cr�ation de l'arbre :
    La mise en commun les deux fr�quences les plus faibles cr�e un nouveau noeud avec une frequence
    �gale a la somme des deux fr�quences.
    Il s'agit ensuite d'ins�rer cette nouvelle structure dans la liste tri�e */
    i = index_frequence;
    while (i != -1)
    {
        ...
        if (struct_inseree == 0)
        {
            ...
            /*printf("Insertion du nouveau noeud � la fin : %d\n", arbre_d[j_save].frequence);*/
        }
        nbre_noeuds++;
...
/*
Proc�dure r�cursive qui cr�e un dictionnaire (correspondance entre la valeur ascii
d'un caract�re et son codage obtenu avec la compression huffman) � partir d'un arbre
*code : pointeur sur une zone m�moire de taille CODE_MAX_LEN recevant
temporairement le code, au fur et � mesure de la progression dans l'arbre
...
/*
Cr�e une table de fr�quences � partir de src, l'inscrit dans dst,
puis affiche un tableau des caract�res de la table, avec leur fr�quence.
*/
char huffman_creer_fichier_frequences(FILE* src, FILE* dst)
{
    ...
    printf("Cr�ation du fichier de fr�quences...\n");
    printf("%d caract�res repr�sent�s sur 256\n", nbre_ascii);

    /*Ecriture de la table des fr�quences */
    while (i != -1)
    {
        nbre_ascii = i;
        fwrite(&nbre_ascii, 1, 1, dst);
...
Le r�sulat se trouve dans le fichier dst
Si frq est diff�rent de NULL, il est utils� pour lire la table des fr�quences
n�cessaire � la construction de l'arbre
*/
char huffman_compacter(FILE* src, FILE* dst, FILE* frq)
{
    ...
    /* cr�ation ou lecture de la table des fr�quences */
    if (frq == NULL)
    {
    ...
    {
        printf("Utilisation d'un fichier de fr�quences pour la cr�ation du dico\n");
    ...
    /* Coinstruction de l'arbre � partir de la table des fr�quences */
    racine_arbre = huffman_creer_arbre(index_frequence);

    /* Allocation de m�moire pour le dictionnaire */
    ...
    /* RAZ du champs taille du dico. Si on utilise une table de fr�quences
    pr�d�finie pour la compression, et qu'un caract�re � compresser n'est pas
    pr�sent dans la table, alors il ne sera pas trait� */
    for (i = 0; i < 256; i++)
        dico[i].taille = 0;

    /* Cr�ation du dictionnaire � partir de l'arbre */
    huffman_creer_dictionnaire(code, racine_arbre, 0);

    /* Compression du fichier source et �criture dans le fichier cible */
    ...
    while ((r = fread(buffer, 1, BUF_LEN, src)) > 0)
    {
        ...
        for (i = 0; i < r; i++)
        {
            /* Ecriture du code correspondant au caract�re dans le dictionnaire */
            octet_r = 0;
            bit_r = 0x80;
...
#ifdef LINUX_COMPIL
    gettimeofday(&tv, NULL);
    printf("Compactage effectu� en %u \xb5s. Taux de compression: %.2f\n", tv.tv_usec - t_debut, (float)nbre_octets / ftell(dst));
#else
    printf("Compactage termin�. Taux de compression : %.2f\n", (float)nbre_octets / ftell(dst));
#endif
    return 0;
}
...
Le r�sultat se trouve dans dst
*/
char huffman_decompacter(FILE* src, FILE* dst, FILE* frq)
{
    ...
    /* Construction de l'arbre � partir de la table des fr�quences */
    racine_arbre = huffman_creer_arbre(index_frequence);

    /* Decompression du fichier source et �criture du r�sultat dans le fichier cible */
    j = racine_arbre;
    /* Lecture de BUF_LEN octets dans le fichier source */
    while ((r = fread(buffer, 1, BUF_LEN, src)) > 0)
    {
...
#ifdef LINUX_COMPIL
    gettimeofday(&tv, NULL);
    printf("D�compression effectu�e en %u\xb5s\n", tv.tv_usec - t_debut);
#else
    printf("D�compression termin�e\n");
#endif
    return 0;
}
...
/*
Fonction principale
Ouvre les fichiers en lecture ou �criture et alloue un espace m�moire suffisant
pour les structures
*/
int main(int argc, char* argv[])
{
    ...
    /* Ouverture du fichier cible en �criture */
    ...
    /* Ouverture d'un �ventuel fichier de liste de fr�quences, en lecture */
    if (argc > 4)
    {
    ...
    /* Allocation m�moire pour les donn�es diverses necessaires � la construction de l'arbre */
    ...
    /* Allocation d'une zone m�moire pour l'arbre */
